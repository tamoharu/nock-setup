...
export function tailnetAddress() {
  ...
  check(s?.BackendState === "Running" && address, "tailscale", "Tailscaleを接続してから招待を表示してください。");
  return address;
}
...
// user's existing SSH keys/options. A lock coordinates simultaneous hati pairers.
export function installPublicKey(sshDirectory, key, requestId) {
  ...
  const lock = join(sshDirectory, ".hati-pair.lock");
  let lockFD;
  try { lockFD = openSync(lock, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY | constants.O_NOFOLLOW, 0o600); }
  catch { throw new Error("別の登録処理が実行中です。.ssh/.hati-pair.lockが残っている場合は、登録処理終了後に確認してください。"); }
  try {
    ...
    try {
      ...
      const line = `${key} hati:${requestId.toLowerCase()}`;
      const existing = before.split("\n").filter((s) => s.endsWith(` hati:${requestId.toLowerCase()}`));
...
export function ephemeralCertificate() {
  const dir = mkdtempSync(join(tmpdir(), "hati-pair-tls-"));
  const keyFile = join(dir, "key.pem"), certFile = join(dir, "cert.pem");
  try {
    const r = spawnSync("openssl", ["req", "-x509", "-newkey", "ec", "-pkeyopt", "ec_paramgen_curve:P-256", "-nodes", "-days", "1", "-subj", "/CN=hati-Pairing", "-keyout", keyFile, "-out", certFile], { stdio: "ignore", timeout: 15000 });
...
export async function createPairing({ address, fingerprints, port = 0, sshPort = 22, name = hostname(), username = userInfo().username,
  ...
  const handle = (request) => {
    check(clock() < expires && !closed, "expired", "QRの有効期限が切れました。PCで hati pair をやり直してください。");
  ...
  // Compact payload keeps the QR readable. Only an explicit CLI --link may print this credential.
  ...
  const qr = "hati://pair/" + Buffer.from(JSON.stringify(invitation)).toString("base64url");
  return { invitation, qr, done, close, address: server.address() };
}
export function copyInvitation(uri, { platform = process.platform, env = process.env, run = spawnSync } = {}) {
  const commands = platform === "darwin" ? [[ ["pbcopy"], ["pbpaste"] ]] : platform === "linux" ? [
    ...(env.WAYLAND_DISPLAY ? [[ ["wl-copy"], ["wl-paste", "--no-newline"] ]] : []),
    ...(env.DISPLAY ? [
      [["xclip", "-selection", "clipboard", "-in"], ["xclip", "-selection", "clipboard", "-out"]],
      [["xsel", "--clipboard", "--input"], ["xsel", "--clipboard", "--output"]],
    ] : []),
  ] : [];
  const execute = ([command, ...args], input) => run(command, args, {
    env, input, encoding: "utf8", timeout: 3000, maxBuffer: 1024 * 1024,
    // Clipboard owners may fork; do not wait on their inherited output pipes.
    stdio: ["pipe", input === undefined ? "pipe" : "ignore", "ignore"],
  });
  for (const [write, read] of commands) {
    if (execute(write, uri).status !== 0) continue;
    return () => {
      const current = execute(read);
      if (current.status === 0 && current.stdout === uri) execute(write, "");
    };
  }
  throw new Error("クリップボードへコピーできません。Linuxではwl-clipboard / xclip / xselが必要です。SSH先などでは hati pair --link で表示してコピーしてください。");
}

export async function pairCommand({ sshPort = 22, noOpen = false, copy = false, link = false } = {}, {
  startPairing = port => createPairing({ address: tailnetAddress(), fingerprints: hostFingerprints(), sshPort: port }),
  copyLink = copyInvitation, log = console.log,
} = {}) {
  const pairing = await startPairing(sshPort);
  let dir, file, clearClipboard;
  const stop = () => pairing.close();
  process.once("SIGINT", stop); process.once("SIGTERM", stop);
  try {
    if (copy || link) {
      if (copy) {
        clearClipboard = copyLink(pairing.qr);
        log("✓ PC用の招待リンクをクリップボードへコピーしました。");
      }
      if (link) log(pairing.qr);
      log("もう一方のPCのHatiで「接続先を追加」に貼り付けてください。5分間・1台限り。接続完了までこのコマンドを開いたままにしてください。Ctrl+Cで中止します。");
    } else {
      dir = mkdtempSync(join(tmpdir(), "hati-pair-qr-"));
      file = join(dir, "pair.html");
      const svg = await QRCode.toString(pairing.qr, { type: "svg", errorCorrectionLevel: "M", margin: 4 });
      writeFileSync(file, `<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="referrer" content="no-referrer"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'"><title>Hati · iPhoneを接続</title><style>body{margin:32px;background:#191a24;color:#eee;font:18px system-ui;text-align:center}svg{width:min(72vh,90vw);height:auto;max-width:680px;background:white}p{color:#b8bed0}</style><h1>Hati · iPhoneを接続</h1><p>iPhoneのHatiで「QRで接続」を開いて読み取ってください。</p><main>${svg}</main><p>5分間・1台限り。QRは共有しないでください。<br>このPCのターミナルを閉じると登録を終了します。</p><script>setTimeout(()=>{document.querySelector('main').textContent='有効期限が切れました。PCで hati pair を実行してください。'},${Math.max(1, pairing.invitation.e - Date.now())})</script></html>`, { mode: 0o600 });
      log("iPhoneのHatiで「QRで接続」を開いてください。5分間・1台限り。Ctrl+Cで中止します。");
      // Screen output can use less damage recovery to reduce the module count.
      log(await QRCode.toString(pairing.qr, { type: "terminal", small: true, errorCorrectionLevel: "L", margin: 2 }));
      if (process.platform === "darwin" && !noOpen) spawnSync("open", [file], { stdio: "ignore" });
    }
    const result = await pairing.done;
    log(result === "paired" ? "✓ 端末の登録とSSH接続が完了しました。" : result === "registered" ? "SSH鍵は登録済みです。接続する端末で再試行してください。" : "登録窓口を閉じました。再試行: hati pair" + (copy ? " --copy" : link ? " --link" : ""));
  } finally {
